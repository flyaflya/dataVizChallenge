...
High altitude changes the physics of how the ball moves through the air and also affects players. The main mechanisms are:

**Primary Mechanisms of High Altitude Effects:**

1. **Reduced air density → less aerodynamic drag**
Batted balls lose less speed, so fly farther on average. This increases the probability a long fly becomes an extra-base hit or a home run.
2. **Reduced Magnus effect → less pitch movement**
Breaking pitches (curveballs, sliders) rely on airflow to generate movement. Thinner air reduces that movement, making breaking pitches flatter and easier to hit.
3. **Temperature and humidity interactions**
Higher temperature and lower humidity (often correlated with altitude and local climate) can further reduce drag and make the ball “livelier.”
4. **Human/strategic effects**
Player endurance and recovery can be impacted at altitude; teams sometimes change roster/strategy (pitching usage, ball handling) to adapt.
Because these physical effects directly change batted-ball distance and pitch movement, stadiums at high elevation tend to show higher run-scoring and home-run rates compared with sea-level parks (all else equal).

**Is Coors Field affected by high altitude?**
Yes. Coors Field in Denver sits at about 5,280 feet above sea level — the highest elevation of any Major League ballpark. That elevation is large enough to produce noticeable effects:

- Historically, Coors Field has had higher rates of runs and home runs per game than most other MLB parks, consistent with reduced drag and reduced pitch movement.
- The Rockies introduced a humidor for baseballs (implemented in 2002) to store balls at controlled humidity. The humidor reduces ball liveliness and mitigates, but does not completely eliminate, the altitude effect.
- Other non-altitude factors (park dimensions, wind, team composition, rule changes, league-wide offense trends) also influence run rates, so altitude is an important but not sole contributor.

**How to test this with your dataset (quick suggestions)**
- Compare Coors Field ("COL") vs league: compute average runs/game and HR/game and show percent above league average.
- Visual checks: horizontal bar charts (or two-facet 2010 vs 2021) that highlight "COL".
- Statistical checks: simple t-test or bootstrap confidence intervals for runs/HR per game between COL and other parks; compare pre-/post-humidor if you have earlier seasons.
:::
...
**Answer:**
There are 2,430 games in the 2010 file and 2,429 games in the 2021 file (4,859 games total). We aggregate individual games into stadium averages for several practical and statistical reasons:

- Statistical stability: single games are noisy (pitcher matchups, weather, randomness). Averaging across all home games at a stadium reduces variance and reveals the persistent stadium-level effect.
- Signal vs noise: aggregation amplifies systematic park effects (altitude, dimensions, humidor) and de-emphasizes transient game-to-game noise.
- Appropriate unit of inference: the business question is about the stadium ("Is Coors Field run-friendly?"), so stadium-level averages match the decision unit.
- Readability & communication: ~30 stadium points are easier to read and compare (rankings, percent-above-league) than thousands of game-level points.
- Visualization design: aggregates prevent overplotting and let you use accurate encodings (bar lengths, ordered lists, annotations) that support precise comparisons.

Below is a small, ready-to-paste pandas snippet that computes per-stadium game counts and averages for runs and home runs (useful before plotting):

```{python}
# Compute per-stadium averages and game counts
agg_2010 = (df2010
    .assign(totalRuns = df2010.homeScore + df2010.visScore,
            totalHR = df2010.homeHR + df2010.visHR)
    .groupby('home', as_index=False)
    .agg(games_played=('date','count'),
         avg_runs=('totalRuns','mean'),
         avg_hr=('totalHR','mean'))
    .sort_values('avg_runs', ascending=False)
)

agg_2021 = (df2021
    .assign(totalRuns = df2021.homeScore + df2021.visScore,
            totalHR = df2021.homeHR + df2021.visHR)
    .groupby('home', as_index=False)
    .agg(games_played=('date','count'),
         avg_runs=('totalRuns','mean'),
         avg_hr=('totalHR','mean'))
...
print('2010 games:', df2010.shape[0], '→ stadiums:', len(agg_2010))
print('2021 games:', df2021.shape[0], '→ stadiums:', len(agg_2021))
print('\nTop stadiums 2010 by avg runs:\n', agg_2010.head())
```
...
**Answer:**
**Horizontal vs. scatter / vertical (concise answer):**

- Horizontal bar charts are better here because stadiums are categorical and bar length encodes the numeric value (average runs) using position/length — a highly accurate perceptual channel. Scatter plots imply two continuous variables and are less clear when the x-axis is just category labels; they also make reading and comparing many category labels awkward.
- Use a vertical bar chart when you have few categories, short labels, or when the x-axis encodes an ordered or temporal variable (time series). Horizontal bars are preferable for many categories, long labels, or ranking tasks because labels are easier to read and comparisons remain precise.
...
**Answer:**
**Color highlighting (Approach 4)**

- Creates a clear visual hierarchy: the highlighted stadium (COL) immediately becomes the focal point while other stadiums recede to secondary importance.
- Improves pattern detection: color helps viewers find the stadium of interest quickly without scanning labels or bar lengths.
- Supports storytelling: it emphasizes the specific claim you're testing (e.g., "Is Coors Field unusually run-friendly?") and makes the intended comparison explicit.
- Works as a visual filter: color can be used to group or flag categories (highlight, above/below average, region), aiding viewers who skim the chart.

**Position vs. Color for encoding data**

- Position (x/y) is the most accurate perceptual channel for quantitative values. Use position (bar length or axis position) for the primary numeric variable (average runs) when precise comparison is required.
- Color is best used as a secondary, categorical or emphasis channel. It quickly draws attention and encodes group membership but is less precise for reading exact numeric differences.
- Combine them: encode the numeric value with position and use color to add emphasis or category information (e.g., highlight COL in purple while encoding runs with bar length). This preserves accuracy while supporting narrative emphasis.
- Accessibility note: when relying on color, ensure sufficient contrast and provide redundant encodings (labels, annotations, or shape) for colorblind or grayscale viewers.
...
**Answer:**
- Clear, audience-first headline: a bold title that states the key takeaway so executives immediately understand the message.
- Single, prioritized focal point: Colorado (COL) is highlighted with a contrasting color so stakeholders spot the subject of interest at a glance.
- Precise quantitative encoding: bar length (position) is used for the main metric (average runs), which supports accurate comparisons.
- Clean, professional styling: a restrained color palette, readable fonts, consistent label formatting, and light gridlines that aid reading without clutter.
- Contextual reference lines and annotations: league-average lines and annotated COL values provide context and the exact numbers decision-makers want.
- Redundant encodings for accessibility: numeric labels, legend, and annotations supplement color so the insight works in grayscale and for color-vision deficiencies.
- Balanced layout and spacing: readable figure size, tight layout, and proper margins make the chart slide-ready.
- Executive summary and actionable insight: a short textual takeaway (one or two sentences) below or beside the chart translates data into business implications and recommendations.
- Comparability across facets: consistent sorting and shared x-axis when comparing 2010 vs 2021 so viewers can make fair comparisons quickly.
- Clean exportability: use of object-oriented matplotlib to control exact layout and styling ensures the same appearance when exported to slides or printed reports.
...
explanation_text = (
    ...
    f"2010: {col_runs_2010:.2f} runs/game (+{((col_runs_2010/league_avg_2010)-1)*100:.1f}% vs league) • "
...
::: {.callout-warning}
## ❌ **NO - AI Makes a Critical Factual Error**
...
### 🎓 **Lesson for AI Collaboration**

This demonstrates why human oversight is crucial when working with AI:

- ✅ **Always verify AI's conclusions against the actual visualizations**
- ✅ **Don't let narrative convenience override factual accuracy**
- ✅ **Use your own eyes to check rankings and positions in charts**
- ✅ **Question claims that seem too perfectly aligned with initial hypotheses**
...
## 🎯 **Mastery Demonstration: Advanced Object-Oriented Matplotlib Analysis**
...
ax2.set_title('Stadium Performance Change: 2010 → 2021', fontsize=14, fontweight='bold', color=colors['text'])
...
ax4.set_title('Stadium Ranking Changes: 2010 → 2021', fontsize=14, fontweight='bold', color=colors['text'])
...
print(f"\n📊 LEAGUE-WIDE TRENDS:")
print(f"   • League average decreased by {((1 - league_avg_2021/league_avg_2010)*100):.1f}% from 2010 to 2021")
print(f"   • {len(comparison_data[comparison_data['runs_change'] > 0])} stadiums improved scoring")
print(f"   • {len(comparison_data[comparison_data['runs_change'] < 0])} stadiums decreased scoring")

print(f"\n🏟️ COLORADO ROCKIES ANALYSIS:")
print(f"   • 2010: {col_runs_2010:.2f} runs/game ({((col_runs_2010/league_avg_2010)-1)*100:+.1f}% vs league)")
print(f"   • 2021: {col_runs_2021:.2f} runs/game ({((col_runs_2021/league_avg_2021)-1)*100:+.1f}% vs league)")
print(f"   • Change: {col_change:+.2f} runs/game ({((col_runs_2021/col_runs_2010)-1)*100:+.1f}%)")
print(f"   • Ranking: {comparison_data[comparison_data['stadium'] == 'COL']['rank_2010'].iloc[0]:.0f} → {comparison_data[comparison_data['stadium'] == 'COL']['rank_2021'].iloc[0]:.0f}")

print(f"\n🎯 KEY INSIGHTS:")
print(f"   • Colorado maintains above-average scoring in both seasons")
print(f"   • Altitude effects remain significant despite league-wide changes")
print(f"   • Stadium effects vary significantly across MLB venues")
print(f"   • Multiple factors beyond altitude influence run production")
```
...
🏟️ STADIUM PERFORMANCE
• Colorado maintains above-average run production
• 2010: {ax1_data['runs_2010']:.2f} runs/game (+{((ax1_data['runs_2010']/league_avg_2010)-1)*100:.1f}% vs league)
• 2021: {ax1_data['runs_2021']:.2f} runs/game (+{((ax1_data['runs_2021']/league_avg_2021)-1)*100:.1f}% vs league)

📈 BUSINESS IMPLICATIONS
• Altitude effects remain significant factor
• Consistent above-average performance
• Fan experience: Higher scoring games
• Team strategy: Offense-friendly environment

🎯 KEY RECOMMENDATIONS
• Leverage altitude advantage in player acquisition
• Market high-scoring games to fans
• Consider humidor adjustments if needed
• Monitor league-wide scoring trends
"""
